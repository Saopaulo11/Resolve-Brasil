...
export async function createCase(input: {
  ...
  /*
   * Выбрал «Problema com Pix» — значит платил через Pix (§5, §36).
   *
   * Это не догадка модели, а то, что человек сказал сам, выбрав категорию:
   * источник USER_FACT. Без этого блок с ситуацией по Pix появлялся бы
   * только после того, как способ оплаты извлекут из документов, то есть
   * после разбора и вложений, — а спросить «что именно случилось с Pix»
   * нужно сразу: от ответа зависит, куда дело вообще идёт, и по рассказу
   * мошенничество от коммерческого спора не отличить.
   */
  if (input.category === "PROBLEMA_COM_PIX") {
    await cases.setFields(created.id, { paymentMethod: "PIX" });
    created.paymentMethod = "PIX";
  }
...
/**
 * Доступ к делу для того, кто его только что завёл и ещё не вошёл (§15).
 *
 * Зачем: человек рассказывает о проблеме и сразу должен увидеть разбор.
 ...
 *
 * Чем это не дыра. Номер дела приходит из подписанной куки: подделать её
 * нельзя, она httpOnly и недоступна скрипту, живёт полчаса и содержит ровно
 * одно дело — то, которое человек завёл в этом браузере.
 *
 * И главное: как только у дела появляется владелец, кука перестаёт давать к
 * нему доступ. Иначе на общем телефоне следующий человек увидел бы чужое
 * дело. Поэтому здесь жёстко userId === null, а не «совпадает или пусто».
 */
export async function getUnclaimedCase(
  publicId: string,
  cookieCaseId: string | null,
): Promise<CaseWithTimeline | null> {
  if (!cookieCaseId || cookieCaseId !== publicId) return null;

  const { cases } = stores();
  const found = await cases.findByPublicId(publicId);
  if (!found) return null;

  // Дело уже чьё-то — кука на него прав не даёт.
  if (found.userId !== null) return null;
